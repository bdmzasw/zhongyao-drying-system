...
def herb_dry_selection(herb_name, area_name, annual_output=800, custom_weights=None):
    try:
        ...
        herb_min_temp, herb_max_temp = parse_herb_temp(herb_temp_str)

        df_tech_filter = df_tech.copy()

        df_tech_filter["部位匹配"] = df_tech_filter["适用药材部位"].apply(
            lambda x: "通用" in x or herb_part in x
        )
        df_tech_filter = df_tech_filter[df_tech_filter["部位匹配"]].reset_index(drop=True)
        if df_tech_filter.empty:
            return None, None, None, "无匹配的药材部位干燥技术！"

        df_tech_filter["温度匹配"] = df_tech_filter.apply(
            lambda x: check_temp_match(herb_temp_str, x["适用最低温(℃)"], x["适用最高温(℃)"]),
            axis=1
        )
        df_tech_filter = df_tech_filter[df_tech_filter["温度匹配"]].reset_index(drop=True)
        if df_tech_filter.empty:
        ...
        df_tech_filter["综合得分"] = (
            weights["成本"] * df_tech_filter["成本归一化"] +
            weights["碳排放"] * df_tech_filter["碳排归一化"] +
            weights["药效保留"] * (1 - df_tech_filter["药效归一化"])
        )
        df_tech_filter["综合得分"] = df_tech_filter["综合得分"].round(6)
        ...
        best_costs = df_result.iloc[0][["设备折旧(万元)", "能耗成本(万元)", "碳交易成本(万元)", "综合成本(万元)"]] if (not df_result.empty and best_tech) else None
...
def sensitivity_analysis(herb_name, area_name, base_param, fluct_ratios=[-0.2, -0.1, 0, 0.1, 0.2]):
    try:
        df_result, best_tech, _, _ = herb_dry_selection(herb_name, area_name, BASE_PARAMS["年产量"])
        ...
        base_cost, _, _, _ = calc_total_cost(tech_row, area_row, herb_tons_water, annual_output=BASE_PARAMS["年产量"])
        if base_cost < 1e-6:
            base_cost = 1e-6

        sensi_data = []
        for fr in fluct_ratios:
            fluct_val = round(base_val * (1 + fr), 3)
            cost = base_cost

            if base_param == "电价":
                area_fluct = area_row.copy()
                area_fluct["工业电价(元/度)"] = fluct_val
                cost, _, _, _ = calc_total_cost(tech_row, area_fluct, herb_tons_water, BASE_PARAMS["年产量"])
            elif base_param == "年产量":
                cost, _, _, _ = calc_total_cost(tech_row, area_row, herb_tons_water, annual_output=fluct_val)
            elif base_param == "设备投资":
                tech_fluct = tech_row.copy()
                tech_fluct["设备平均投资(万元)"] = fluct_val
                cost, _, _, _ = calc_total_cost(tech_fluct, area_row, herb_tons_water, BASE_PARAMS["年产量"])
            elif base_param == "能耗系数":
                tech_fluct = tech_row.copy()
                tech_fluct["平均能耗系数"] = fluct_val
                cost, _, _, _ = calc_total_cost(tech_fluct, area_row, herb_tons_water, BASE_PARAMS["年产量"])
            elif base_param == "碳交易价格":
                area_fluct = area_row.copy()
                area_fluct["碳交易价格（元）"] = fluct_val
                cost, _, _, _ = calc_total_cost(tech_row, area_fluct, herb_tons_water, BASE_PARAMS["年产量"])

            cost_change = round((cost - base_cost)/base_cost * 100, 2) if base_cost !=0 else 0
            sensi_data.append({
                "分析参数": base_param,
                "波动比例(%)": f"{fr*100:.0f}",
                "参数值": fluct_val,
                "综合成本(万元)": round(cost, 3),
                "成本变化率(%)": cost_change
            })
...
def full_sensitivity_analysis(herb_name, area_name):
    ...
    results = []
    for p in params:
        df_s = sensitivity_analysis(herb_name, area_name, p)
        if df_s.empty or len(df_s) < 2:
            continue
        df_calc = df_s[pd.to_numeric(df_s["波动比例(%)"]) != 0]
        if not df_calc.empty and len(df_calc) > 0:
            coeff = round((pd.to_numeric(df_calc["成本变化率(%)"]) / pd.to_numeric(df_calc["波动比例(%)"])).mean(), 3)
            if abs(coeff) >= 0.9:
                level = "🔴 最敏感"
            elif abs(coeff) >= 0.5:
                level = "🟡 高敏感"
                ...
                level = "🟢 最不敏感"
            if p == "年产量":
                base_val_show = BASE_PARAMS[p]
            else:
                base_val_show = round(BASE_PARAMS[p], 2)
            results.append({
                "参数": p,
                "基准值": base_val_show,
                "敏感度系数": coeff,
...
with tab1:
    st.subheader("1. 药材库")
    st.dataframe(df_herb, hide_index=True)
...
with tab2:
    ...
    if total_weight == 0:
        ...
        custom_weights = {
            "成本": round(cost_weight / total_weight, 3),
    ...
    if st.button("开始选型", type="primary"):
        ...
        if df_result is None or best_tech is None:
            st.error(f"❌ {msg}")
        else:
            ...
            show_cols = ["技术名称", "综合成本(万元)", "综合碳排(吨)", "药效保留率", "成本归一化", "碳排归一化", "综合得分"]
            st.dataframe(df_result[show_cols], hide_index=True)
            ...
            if best_costs is not None:
                ...
                ax2.pie(df_cost["金额"], labels=df_cost["成本类型"], autopct='%1.2f%%', startangle=90,
                        colors=["#95E1D3", "#FCE38A", "#FF8A80"], wedgeprops=dict(edgecolor="white", linewidth=2))
                ax2.axis('equal')
                st.pyplot(fig2)
...
with tab3:
    ...
    if best_tech is None:
        ...
        sensi_param = st.selectbox(
            "选择分析参数",
            ["电价", "年产量", "设备投资", "能耗系数", "碳交易价格", "全参数汇总"]
        )
        ...
        if st.button("开始分析", type="primary"):
            with st.spinner("正在计算参数波动对成本的影响..."):
                if sensi_param == "全参数汇总":
                    ...
                    st.dataframe(df_sensi, hide_index=True)
                    ...
                    if not df_calc.empty and len(df_calc) > 0:
                        ...
                        if abs(avg_coeff) >= 0.9:
                            level = "🔴 最敏感"
                        elif abs(avg_coeff) >= 0.5:
                            level = "🟡 高敏感"
                        elif abs(avg_coeff) >= 0.1:
                            level = "⚪ 一般"
                        else:
                            level = "🟢 最不敏感"
                        st.success(f"平均敏感度系数：{avg_coeff} | {level}")
...
with tab4:
    ...
    if best_tech is None:
        ...
        with col1:
            ...
            ax1.pie(df_weight_plot["权重"], labels=df_weight_plot["指标"], autopct='%1.1f%%', startangle=90,
                    colors=["#4B9CD3", "#FFD700", "#FF6B6B"], wedgeprops=dict(edgecolor="white", linewidth=2))
            ax1.axis('equal')
            st.pyplot(fig1)
        
        with col2:
            # 修复列名不匹配：直接显示原始转置表格，不手动改列名
            st.markdown("### 📍 区域动态适配因子")
            adapt_cols = ["综合修正因子", "电网碳排放因子", "碳交易价格（元）", "工业电价(元/度)"]
            df_adapt_show = pd.DataFrame(area_row[adapt_cols]).T
            st.dataframe(df_adapt_show, hide_index=True)
        ...
        if not df_sensi_dash.empty:
            st.dataframe(df_sensi_dash, hide_index=True)
        else:
